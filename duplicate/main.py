...
class DuplicatesGUI(Frame):
    ...
    def extensions_entry_callback(self, event):
        ...
        extensions = [item.strip(' .') for item in re.split(',|;| ', text) if item.strip()]
        ...
        for key, values in duplicates:
            ...
            for index, filepath in enumerate(values):
                ...
                remove_box = Checkbutton(
                    ...
                    offvalue=0,
                    anchor=W,
                    command=lambda path=filepath, status=box_status: self.checkbox_command(path, status)
                    )
                remove_box.bind("<Button-3>", lambda event, path=Path(filepath).parent: self.popup(event, path))
    ...
    def run_gui(self):
        ...
        directory_button = Button(entries_text_frame, relief=self.relief, text='path...', command=self.choose_and_set_directory)
        ...
        # search button
        search_button = Button(topbar, relief=self.relief, text='search', command=self.spawn_thread)
        search_button.pack(expand=YES, fill=BOTH, side=LEFT)
        
        # clear button
        search_button = Button(topbar, relief=self.relief, text='clear', command=self.general_cleanup)
        search_button.pack(expand=YES, fill=BOTH, side=LEFT)
        
        # select n-1
        select_almost_all_button = Button(topbar, relief=self.relief, text='select\nn-1', command=self.select_almost_all)
        select_almost_all_button.pack(expand=YES, fill=BOTH, side=LEFT)
        
        # remove-selected button
        remove_button = Button(topbar, relief=self.relief, text='remove\nselected', fg='red', activeforeground="red", command=self.remove_selected_files)
        remove_button.pack(expand=YES, fill=BOTH, side=LEFT)
        
        # use wrapper
        self.vertical_scrolled_frame = VerticalScrolledFrame(self.master, relief=self.relief, bg=self.widgets_bg_color)
    ...
    def popup(self, event, path):
        ...
        popup_menu.add_command(label="open directory", command=lambda: self.open_directory(path))
        try:
